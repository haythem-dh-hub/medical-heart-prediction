...
def cholesterol_scatter(records: pd.DataFrame):
    ...
    if not PLOTLY_AVAILABLE:
        return ChartFallback(
            ...
            sample[["patient_id", "chol", "trestbps", "age", "risk_band", "department"]].head(80),
        )
    ...
    fig = px.scatter(
        ...
        color="risk_band",
        size="age",
        hover_data=["department", "patient_id"],
...
def correlation_heatmap(records: pd.DataFrame):
    numeric = records[["age", "trestbps", "chol", "thalach", "oldpeak", "bmi", "glucose", "target"]]
